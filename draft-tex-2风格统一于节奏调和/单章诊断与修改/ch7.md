...
下面是需要诊断和改写的 LaTeX 文本：% !TEX root = AImath.v5.tex
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figs/大脑神经细胞的工作流程.jpg}
  \caption{大脑神经细胞的工作流程：树突接收、胞体整合、阈值触发、轴突传递。}
  \label{大脑神经细胞的工作流程}
\end{figure}

如图~\ref{大脑神经细胞的工作流程}所示，上述机制以“接收—整合—触发—传递”的链路循环展开。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figs/neuron.png}
  \caption{生物神经元与人工神经元：从生理结构到数学抽象的映射。}
  \label{生物神经元与人工神经元}
\end{figure}

如图~\ref{生物神经元与人工神经元}，生物神经元的生理结构与人工神经元的数学抽象之间具有一一映射关系。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figs/history.png}
  \caption{人工神经网络发展史：从 M-P 模型到感知机与多层网络，早期以逻辑门抽象神经元行为。}
  \label{人工神经网络发展史}
\end{figure}

如图~\ref{M-P神经元模型}与图~\ref{M-P模型的另一种描述}展示了同一思想的两种表达：加权求和与阈值判定可等价地视为逻辑门的激活机制。
...
\begin{figure}[htbp]
  ...
  \caption{M-P神经元模型：加权求和与阈值判定的二值输出。}
  \label{M-P神经元模型}
\end{figure}

如图~\ref{罗森布拉特提出的感知机模型}所示，误差反馈驱动的参数更新使感知机具备自我修正能力。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figs/M-P模型的另一种描述.jpg}
  \caption{M-P模型的另一种描述：以逻辑门视角呈现激活机制。}
  \label{M-P模型的另一种描述}
\end{figure}

如图~\ref{感知机的超平面}，线性可分情况下的分类边界可被单层感知机学习到。
...
\begin{figure}[htbp]
  ...
  \caption{罗森布拉特提出的感知机模型：引入基于误差反馈的参数学习机制。}
  \label{罗森布拉特提出的感知机模型}
\end{figure}

如图~\ref{感知机学习算法示意图}，权重更新遵循“预测—误差—修正”的闭环流程。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figs/感知机的超平面.jpg}
  \caption{感知机的超平面：线性可分条件下的分类决策边界。}
  \label{感知机的超平面}
\end{figure}

如图~\ref{MLP}，多层结构通过隐含层引入非线性，使模型具有更强的表达能力。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figs/感知机学习算法示意图.jpg}
  \caption{感知机学习算法示意图：基于误差的权重更新流程。}
  \label{感知机学习算法示意图}
\end{figure}

如图~\ref{前馈神经网络的输出}，层级映射下的决策函数体现了特征的逐层抽象与组合。
...
\begin{figure}[htbp]
  ...
  \caption{多层感知机网络：通过隐含层实现非线性特征的分层抽象。}
  \label{MLP}
\end{figure}

如图~\ref{图像分类}与图~\ref{文本分类}所示，端到端的深度网络可在视觉与语言两类任务中统一“特征—语义—决策”的建模路径。

\begin{figure}[htbp]
  \centering
  %（原图缺失，暂移除以避免编译错误；如需保留“异或”示意，请提供素材或改用现有图）
  % \includegraphics[width=0.9\textwidth]{figs/可解决“异或”问题的两层感知机.jpg}
  % \caption{两层感知机可解决“异或”问题：隐含层引入非线性分割能力。}
  % \label{fig:可解决“异或”问题的两层感知机}
\end{figure}

如图~\ref{通用逼近定理}所示，尽管浅层网络在理论上拥有逼近能力，但工程上的规模与效率瓶颈仍需深度结构来缓解。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figs/前馈神经网络的输出.jpg}
  \caption{前馈神经网络的输出：层级映射下的决策函数示意。}
  \label{前馈神经网络的输出}
\end{figure}

如图~\ref{activation}所示，激活函数的选择直接影响网络的可训练性与表达空间。

% RESTORED from base
% 常见任务示例：图像分类与文本分类
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figs/图像分类.png}
  \caption{图像分类：从像素到语义的层次化理解。}
  \label{图像分类}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figs/文本分类.png}
  \caption{文本分类：词向量—语义组合—决策映射的端到端流程。}
  \label{文本分类}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figs/通用逼近定理.jpg}
  \caption{通用逼近定理：具有足够隐藏单元的浅层网络可以逼近任意连续函数。}
  \label{通用逼近定理}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{常见激活函数：Sigmoid、Tanh、ReLU 等的曲线形状与性质。}
  \label{activation}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figs/认知.png}
  \caption{认知世界：我们对自身了解多少？从感知到理解的层次化进步仍在持续。}
